...
function Newbill() {
  useEffect(() => {
    document.title = "MakeMyBill.com | Make Bill";
  }, []);
  ...
  async function submitForm() {
    ...
    if (
      ...
      const totalAmount =
        ...
        parseFloat(pkgs) * parseFloat(price) -
        parseFloat(advance);
      ...
      if (
        ...
        sessionStorage.getItem("receiverState")
      ) {
      ...
      try {
        let response = await axios.get(
          `http://localhost:8080/bill-details/${vehicle}/${name}/${add}/${gst}/${pkgs}/${price}/${booking}/${advance}/${gstCalculated}/${totalCalculated}/${location}/${parseInt(
