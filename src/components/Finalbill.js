...
function Finalbill() {
  ...
  useEffect(() => {
    document.title = "MakeMyBill.com | Fianlbill";
  }, []);

  // Getting Latest Bill details

  let response = 0;
  useEffect(async function getBill() {
    try {
      setLoading(true);
      response = await axios.get(
        `http://localhost:8080//getLatestBill/${sessionStorage.getItem(
          "user_id"
        )}`
      );
        ...
        >
          ...
          <Segment>
            <Segment textAlign="left" style={{ marginTop: "0px" }} compact>
              Bill Number - {sessionStorage.getItem("billData")} <br />
              Sending Date - {date}
            </Segment>
              ...
              <Table.Body>
                ...
                <Table.Row>
                  ...
                  <Table.Cell>
                    +
                    {Math.round(
                      parseFloat(sessionStorage.getItem("gstAmount")) * 100
                    ) / 100}
                  </Table.Cell>
                </Table.Row>
                ...
                <Table.Row>
                  ...
                  <Table.Cell>-{sessionStorage.getItem("advance")}</Table.Cell>
                </Table.Row>
