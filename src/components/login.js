...
function Login() {
  ...
  useEffect(() => {
    document.title = "MakeMyBill.com | Login";
  ...
  async function submitLogin(e) {
    //On Submit these things needs to be performed
    ...
    try {
      if (authResponse.data === 1) {
        ...
        sessionStorage.setItem(
          "company_address",
          userDetailsResponse.data.company_address
        );
        sessionStorage.setItem(
          "company_name",
          userDetailsResponse.data.company_name
        );
        sessionStorage.setItem(
          "company_state",
          userDetailsResponse.data.company_state
        );
        sessionStorage.setItem(
          "godown_charges",
          userDetailsResponse.data.godown_charges
        );
        sessionStorage.setItem(
          "gst_number",
          userDetailsResponse.data.gst_number
        );
        sessionStorage.setItem(
          "loading_charges",
          userDetailsResponse.data.loading_charges
        );
        sessionStorage.setItem(
          "mobile_number",
          userDetailsResponse.data.mobile_number
        );
        sessionStorage.setItem(
          "other_charges",
          userDetailsResponse.data.other_charges
        );
        sessionStorage.setItem(
          "rtgs_number",
          userDetailsResponse.data.rtgs_number
        );
        sessionStorage.setItem(
          "unioin_charges",
          userDetailsResponse.data.unioin_charges
        );

        // After Fetching Details of User from API store it as session variables

        setLogInLabel(
          <Message color="green" compact basic>
        ...
        setLogInLabel(
          <Message color="red" compact basic>
